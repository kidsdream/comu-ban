...
<?php
  ...
  if (isset($_GET['category'])){
    ...
    $stmt = $dbh->prepare('SELECT id, name, message, created_at FROM messages WHERE category = :category ORDER BY id DESC LIMIT :start, ' . $displayCount);
    $stmt->bindValue(':category', $category, PDO::PARAM_STR);
    $stmt->bindValue(':start', $start, PDO::PARAM_INT);
  } else {
    $stmt = $dbh->prepare('SELECT id, name, message, created_at FROM messages ORDER BY id DESC LIMIT :start, ' . $displayCount);
    $stmt->bindValue(':start', $start, PDO::PARAM_INT);
  }
  $stmt->execute();
?>
